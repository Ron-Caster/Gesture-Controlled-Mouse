...
class HandGestureMouseControl:
    ...
    def process_loop(self):
        ...
        while not self.stop_event.is_set():
            ...
            # --- Sensitivity region mapping ---
            # Define a smaller region (e.g., centered 16:9, 25% of frame size)
            region_w = int(w0 * 0.25)
            region_h = int(region_w * 9 / 16)
            if region_h > h0 * 0.25:
                region_h = int(h0 * 0.25)
                region_w = int(region_h * 16 / 9)
            region_x0 = (w0 - region_w) // 2
            region_y0 = (h0 - region_h) // 2
            region_x1 = region_x0 + region_w
            region_y1 = region_y0 + region_h

            # Move mouse if we have control and pointer present
            if have_control and (px is not None and py is not None):
                # Map only the region to the full screen
                mouse_x = int(np.interp(px, (region_x0, region_x1), (0, self.screen_width)))
                mouse_y = int(np.interp(py, (region_y0, region_y1), (0, self.screen_height)))
